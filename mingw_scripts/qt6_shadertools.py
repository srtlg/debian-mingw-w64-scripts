...
DESCRIPTION = 'Provides functionality for the shader pipeline (mingw-w64)'
PKGNAME = pkgname(NAME, dev=True)
URL = BASE_URL.replace('qtbase-', 'qtshadertools-')


def build():
    if not DPKG.check('qt6-shadertools-dev'):
        raise RuntimeError('requiring qt6-shadertools-dev')
    ...
    dpkg = DPKG(PKGNAME, version=VERSION, description=DESCRIPTION,
                depends=[BASE_PKGNAME, 'qt6-shadertools-dev'])
